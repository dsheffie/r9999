...
`endif
   ...
   generate
	...
	begin : hits
	   ...
	   /* Variable page size (R4000 PageMask): mask off the VPN2 bits that fall
	    * inside the page.  pagemask[11:0] = PageMask[24:13]; a set bit means the
	    * corresponding VA bit (va[13+j]) is part of the page and must NOT be
	    * compared.  vpn[26:12]=va[39:25] is always compared; vpn[11:0]=va[24:13]
	    * is compared only where pagemask==0.  pagemask==0 (4K) -> full compare,
	    * identical to the old behavior (no 4K-path regression). */
   ...
   /* Variable page size (R4000 PageMask), ported from rv64core's discrete-pgsize
    * TLB.  Map the selected entry's pagemask -> a size index 0..6
    * (4K/16K/64K/256K/1M/4M/16M).  Both the even/odd page-pair select bit and the
    * PA offset width scale with the size, using FIXED bit-slices + a mux (no
    * variable shifts) so it stays off the TLB critical path.  4K (pagemask==0)
    * collapses to the old {pfn, va[11:0]}. */
   ...
   /* even/odd select = VA bit just above the single-page offset (bit 12+2k) */
   ...
   /* PA = {pfn high bits, VA page offset}; offset width scales with page size.
    * Each case is exactly PA_WIDTH bits (PFN_WIDTH = PA_WIDTH-12). */
