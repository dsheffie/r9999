#!/bin/bash
# run_l2_pidx_formal.sh -- UNBOUNDED proof of SINGLE-COPY across the L2 + an abstract L1D
...
# cross-module hierarchical references: `dut.w_l1d_pidx` in formal_l1_synonym.sv would
...
DEFS="-DFORMAL -DENABLE_L2_INCLUSION -DENABLE_L2_PIDX -DENABLE_L2_EVICT_BACKINV -DL1SYN_ABSTRACT_L1D $GEOM"

COMMON="
  read_verilog -sv -formal $DEFS -I.. formal_l1_synonym.sv ../l2.sv ../ram2r1w_fwd.sv ../reg_ram1rw.sv ../ram1r1w.sv
  hierarchy -check -top formal_l1_synonym
"
...
echo
echo "=== PDR: unbounded proof of at-most-one-set ==="
...
if grep -q "Property proved" $WORK/pdr.log; then
  echo "  PDR PASS -- at most one L1D alias set ever holds the watched line, in ANY"
  echo "              reachable state (inductive invariant found).  NOTE this is"
  echo "              GIVEN the abstract L1D: the premise that a probe invalidates"
  echo "              the set it names is discharged separately against l1d.sv."
...
if grep -qE 'define-fun \|formal_l1_synonym_a\| .*Bool true' $WORK/l2.smt2; then
...
if grep -qi "unreached" $WORK/cover.log; then
  ...
  echo "  all cover points reached -- the line really is held, at more than one set"
  echo "  across time, and really is probed.  The proof is about a live state space."
fi
