...
`ifdef ENABLE_DEBUG_WATCHPOINT
    ...
    * letting GPRs/DRAM be read coherently at the offending instruction. bp_pc is now
    * driver-programmable (slv_reg9) so the breakpoint can be aimed without a re-synth. */
   logic	r_bp_hit;
    ...
    * remember its rob_ptr (r_wp_pending/r_wp_robptr); when that rob_ptr's data arrives and
    * equals bp_wp_val, FREEZE -- i.e. freeze on the store that writes the CORRUPT value to
    * the watched slot, whichever instruction did it.  The frozen retire PC names its region
    * (the pc field is sim-only, so we read it from the retire stream, not the mem req). */
   logic	r_wp_hit;
   logic	r_wp_pending;
   ...
   /* bp_wp_val == 0xffffffff is a WILDCARD: freeze on ANY store to bp_wp_addr (address-only
    * capture, e.g. to trace every write to a malloc chunk-header word). */
...
`else
   ...
   wire		w_step_ok   = (r_single_step | r_fault_hit) ? (t_step_edge) : 1'b1;
`endif
...
`ifdef ENABLE_DEBUG_WATCHPOINT
	/* fault_clear (ctrl bit18) doubles as bp resume: clears r_bp_hit so the
	 * core runs full-speed to the NEXT bp_pc match (re-latches next hit). */
...
`endif
   ...
   /* ---- resettable fault-trap ------------------------------------------------
    * While armed (bp_enable), the FIRST fatal USERSPACE arch-fault (AdEL/AdES/IBE/
    * DBE/RI, EPC in useg) latches {epc,cause,badvaddr} and sets r_fault_hit, which
    * folds into w_step_ok below to FREEZE the core -- so the ARM can read a coherent
    * fault state (the latched taps override the live epc/badvaddr/cause outputs so
    * the read is STABLE, not clobbered by the kernel handler).  fault_clear clears
    * the latch + re-arms + un-freezes -> the ARM STEPS fault->fault. */
   logic		    r_fault_hit;
...
`else
   assign dbg_frozen = {2'b00, r_fault_hit};
`endif
